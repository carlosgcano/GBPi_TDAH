...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def keyPressEvent(self, event):    
        if event.key() == QtCore.Qt.Key_Down and self.stackedWidget.currentIndex()==0 and self.trophies_points>0:
            ...
            if self.Lista_juegos.currentRow() == 0:
                self.stackedWidget.setCurrentIndex(0)
            else:
                self.Lista_juegos.setCurrentRow (self.Lista_juegos.currentRow()-1)
        elif event.key() == QtCore.Qt.Key_Down and self.stackedWidget.currentIndex()==1:
            if self.Lista_juegos.currentRow() < 0:  
                self.Lista_juegos.setCurrentRow (0)
            else:
                if(self.Lista_juegos.currentRow() < self.Lista_juegos.count()-1):
                    self.Lista_juegos.setCurrentRow (self.Lista_juegos.currentRow()+1)
        elif event.key() == QtCore.Qt.Key_B and self.stackedWidget.currentIndex()==1:         
            self.execute_game()
            
    def execute_game(self):      
        os.system("timeout -k 10 "+self.game_time+"s  /usr/bin/vba games/"+self.Lista_juegos.currentItem().text()+".gb")
        juego=self.Lista_juegos.currentItem().text()+".gb"
...
if __name__ == "__main__":
    ...
    if window.trophies_points < 1:
        ...
        for index in range(window.Lista_juegos.count()-window.trophies_points):
            window.Lista_juegos.takeItem(0)
